...
public class SystemNotificationFragment extends Fragment {
			...
			public void run() {
				while (true) {
					...
					try {
						...
						Log.d(":(",
								"exception inside 2 min refresher ::\n"
										+ e.getCause() + "\n" + e.getMessage()
										+ "\n" + e.toString());
					}
				}
	...
	private static void notifyUser(NotifItem notifItem) {
		NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(
				...
				.setContentText(
						notifItem.getDatetime() + " "
								+ notifItem.getDescription() + ", "
								+ notifItem.getClientName() + ", "
								+ "Unassigned");
			...
			if (responseObj != null) {
				try {
					...
					if (status.equals("OK")) {
						...
						ArrayList<NotifItem> notifList2 = NotifItem
								.parseNotifList(notifArray);
						Log.e("???????",
								"notifications count = " + notifList2.size());
						...
						} else if (!doesNotifListContain(notifList2
								.get(notifList2.size() - 1))) {
