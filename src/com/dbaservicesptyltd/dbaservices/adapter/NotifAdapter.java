...
public class NotifAdapter extends ArrayAdapter<NotifItem> {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (severity == Constants.NOTIF_SEVERITY_ALERT) {
			holder.tvNotif.setTextColor(tContext.getResources().getColor(
					R.color.red));
	...
	public void addData(List<NotifItem> notifList) {
		if (notifList != null) {
			for (int i = 0; i < notifList.size(); i++) {
				add(notifList.get(i));
			}
