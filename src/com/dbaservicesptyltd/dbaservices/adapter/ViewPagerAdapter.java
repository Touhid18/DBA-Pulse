...
public class ViewPagerAdapter extends FragmentPagerAdapter {// implements
	...
	public Object instantiateItem(ViewGroup container, int position) {
		if(position==3)
			return new JobsInProgressFragment(tContext, admin, adminClickListener);
	...
	public void addJobPage(ViewPager pager, OnlineAdminRow admin) {
		...
		Fragment jobsFrag = new JobsInProgressFragment(tContext, admin, adminClickListener);
		...
		if (manager != null) {
			...
			Log.d("......", "manager null");
			FragmentTransaction trans = ((FragmentActivity) tContext).getSupportFragmentManager().beginTransaction();
			trans.add(jobsFrag, fTag).commit();
			pager.addView(jobsFrag.onCreateView(
					(LayoutInflater) tContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE), pager, null));
		}
